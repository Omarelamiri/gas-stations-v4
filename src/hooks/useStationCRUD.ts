...
import { transformNewStationToFirestore, transformStationForUpdate } from '@/lib/utils/stationTransformers';
...
export function useStationCRUD() {
  ...
  const createStation = useCallback(async (stationData: NewGasStation): Promise<void> => {
    ...
    try {
      await addDoc(collection(db, 'gasStations'), firestoreData);
    } catch (error) {
  ...
  const updateStation = useCallback(async (
    ...
    try {
      await updateDoc(doc(db, 'gasStations', stationId), firestoreData);
    } catch (error) {
  ...
  const deleteStation = useCallback(async (stationId: string): Promise<void> => {
    ...
    try {
      await deleteDoc(doc(db, 'gasStations', stationId));
    } catch (error) {
