...
import { GasStation, GasStationFormData, StationType, StationAuthorization } from "@/types/station";
...
/**
 * Convert Firestore document → GasStation
 */
...
/**
 * GasStation → form data (all strings)
 */
export function stationToFormData(s: GasStation): GasStationFormData {
  return {
    ...
    "Date Creation": s["Date Creation"]
      ? s["Date Creation"].toISOString().slice(0, 10)
      : "",
    "numéro de création": s["numéro de création"] ?? "",
    "Date Mise en service": s["Date Mise en service"]
      ? s["Date Mise en service"].toISOString().slice(0, 10)
      : "",
...
/**
 * Form data → Firestore payload
 */
export function formDataToFirestore(
  input: GasStationFormData
): Record<string, any> {
